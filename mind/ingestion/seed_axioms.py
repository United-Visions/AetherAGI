...
AETHER_AXIOMS = [
    ...
    # ==================================================
    # AUTONOMOUS TASK COMPLETION SYSTEM (AGI-Level Persistence)
    # ==================================================
    {"subject": "AetherMind", "topic": "GoalTracker",         "content": "GoalTracker stores complex goals with subtasks in Supabase. Goals persist across sessions, server restarts, and browser closures. Each goal has UUID, user_id, description, status (pending/in_progress/completed/failed), priority (1-10), subtasks array with dependencies, and metadata. Subtasks track: action_type, action_params, dependencies, status, attempt_count, max_attempts, execution_result, error_message. Goals table has RLS policies for user isolation and system worker access."},
    
    {"subject": "AetherMind", "topic": "AutonomousAgent",     "content": "AutonomousAgent provides self-healing continuous execution. Main loop: 1) Load goal from Supabase, 2) Check for actionable subtasks (dependencies met), 3) Execute subtask via ActionParser, 4) If failure: generate fix strategy by asking Brain to analyze error, 5) Retry with updated approach (max 3 attempts), 6) Update subtask status, 7) Continue until all subtasks completed or failed. Max 50 iterations per goal. Agent never gives up - adapts strategy based on actual execution results."},
    
    {"subject": "AetherMind", "topic": "BackgroundWorker",    "content": "BackgroundWorker runs independently as asyncio task, polling Supabase every 30 seconds for pending goals. On startup (main_api.py lifespan), worker starts automatically. When new goal detected: 1) Check if subtasks exist, if not decompose goal into subtasks via Brain, 2) Spawn AutonomousAgent in separate task, 3) Agent works until completion. Worker survives FastAPI reloads, server restarts, and user logouts. Graceful shutdown on SIGINT/SIGTERM saves in-progress state."},
    
    {"subject": "AetherMind", "topic": "ExecutionFeedback",   "content": "CRITICAL: Active inference loop now feeds execution results back to Brain. After executing action tags, ActionExecutor returns {success: bool, result: str, output: str, error: str, metadata: dict}. These results stored in session_data['last_execution_results']. On next turn, combined_context includes '## EXECUTION RESULTS FROM PREVIOUS TURN' section showing what actually happened. Brain sees real outcomes (file created, code executed, error thrown) and adapts strategy accordingly. No more hallucinating success - Brain must verify actual results."},
    
    {"subject": "AetherMind", "topic": "GoalDecomposition",   "content": "When user requests complex task (create app, build tool, etc), Brain can decompose into subtasks via AutonomousAgent.decompose_goal_into_subtasks(). Returns array of SubTask objects with: description, action_type (write_file, run_sandbox, install_packages, etc), action_params (file paths, code content, dependencies), dependency IDs (which subtasks must complete first). Brain generates topologically sorted execution plan with explicit dependencies. Example: create_venv → install_deps → write_code → run_tests → commit_git."},
    
    {"subject": "AetherMind", "topic": "MultiTurnExecution",  "content": "Single goal may require 5-15 API calls to Brain. Each turn: 1) Brain sees goal + current subtask + previous results, 2) Brain generates action tags, 3) ActionParser executes and returns detailed results, 4) Results stored for next turn, 5) Brain sees outcomes and continues or adapts. This enables step-by-step verification: Brain doesn't assume success, it sees actual file contents, execution output, error tracebacks. Continues until goal fully completed or max iterations reached."},
    
    {"subject": "AetherMind", "topic": "TaskPersistence",     "content": "Goals survive everything: browser close, server restart, dyno sleep, database migration. State stored in Supabase goals table with JSONB subtasks. On restart, BackgroundWorker queries for in_progress or pending goals and resumes. Each subtask has timestamps (created_at, updated_at, completed_at), attempt_count, and execution_result. Can pause/resume work days later. User closes laptop → goal continues in background → user opens browser next day → sees completed goal."},
    
    {"subject": "AetherMind", "topic": "SelfHealingRetry",    "content": "When subtask execution fails, AutonomousAgent._generate_fix_strategy() asks Brain: 'This subtask failed with error: {error}. Analyze: 1) What went wrong? 2) How to fix? 3) Updated action parameters?'. Brain returns diagnosis and fix. Agent retries with new approach. Example: ImportError → install missing package → retry. SyntaxError → fix code → retry. FileNotFoundError → create directory → retry. Max 3 attempts before marking FAILED and moving to next subtask."},
    
    {"subject": "AetherMind", "topic": "AutonomousAPIs",      "content": "Three API endpoints for goals: 1) POST /v1/goals/create (description, priority) → returns goal_id, 2) GET /v1/goals/{id}/status → returns goal, subtasks, progress percentage, current step, 3) GET /v1/goals/list (optional status filter) → returns all user goals. Frontend can create autonomous goals from sidebar, track progress in real-time, view detailed execution logs. BackgroundWorker handles all execution asynchronously."},
