...
try:
    ...
    logger.warning("torch not available - using simplified reward model")
...
if TORCH_AVAILABLE:
    ...
    # Simplified reward model without torch
    class RewardModel:
        def __init__(self, input_dim=1024, hidden_dim=128):
            """Simplified reward model using heuristics instead of neural network"""
            logger.info("RewardModel (simplified heuristic version) initialized.")
            
        def predict_flourishing(self, state_vector: list) -> float:
            """Simple heuristic: returns neutral score"""
            return 0.5  # Neutral flourishing score
    ...
    def save_model(model, path="heart/weights.pt"):
        logger.info("Simplified reward model - no weights to save")

    def load_model(path="heart/weights.pt", input_dim=1024):
        model = RewardModel(input_dim)
        logger.info("Simplified reward model loaded")
        return model
